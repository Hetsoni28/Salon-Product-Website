            ...
            <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
                  ...
                  <h3 className="font-serif text-xl text-brand-dark">Your cart is empty</h3>
                  <p className="text-gray-500 font-light max-w-[250px]">
                    Looks like you haven't added any professional tools yet.
                  </p>
                  <Button onClick={toggleCart} variant="outline" className="mt-4 rounded-full px-8">
                    Continue Shopping
                  </Button>
                </div>
              ) : (
                <div className="space-y-6">
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-4 p-4 bg-brand-cream-mid/30 rounded-2xl border border-brand-divider">
                      ...
                      <div className="relative w-20 h-24 rounded-xl overflow-hidden bg-white shrink-0">
                        {item.image ? (
                          typeof item.image === 'string' ? (
                            ...
                            <SanityImage image={item.image} alt={item.name} fill className="object-cover" />
                          )
                        ) : (
                      ...
                      <div className="flex flex-col flex-1">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className="font-sans font-semibold text-sm text-brand-dark line-clamp-2 pr-4">{item.name}</h4>
                          <button
                        ...
                        <Price amount={item.price} className="text-sm font-medium mb-3" />
